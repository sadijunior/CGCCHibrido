/*
 * Desafio M6 - Trajetorias Ciclicas dos Objetos
 * Modulo 6 - Computacao Grafica
 */
...
// ============================================================================
// SISTEMA DE TRAJETORIA (ciclica, interpolacao linear parametrica)
// ============================================================================

struct Trajectory {
    vector<vec3> points;       // pontos de controle lidos do arquivo
    int   currentPoint = 0;    // indice do segmento atual
    float t            = 0.0f; // parametro de interpolacao [0,1] dentro do segmento
    float speed        = 0.5f; // velocidade linear (unidades por segundo)
    bool  enabled      = false;

    // Carrega pontos de um arquivo texto (uma linha "x y z" por ponto)
    void load(const string& filePath) {
        ifstream file(filePath);
        if (!file.is_open()) return;  // sem arquivo => trajetoria vazia
        string line;
        ...
        cout << "Trajetoria carregada: " << filePath
             << " (" << points.size() << " pontos)" << endl;
    }

    // Retorna a posicao interpolada no instante atual; avanca o estado interno
    vec3 update(float deltaTime) {
        if (points.size() < 2) return points.empty() ? vec3(0.0f) : points[0];
        int next = (currentPoint + 1) % (int)points.size();
        vec3 from = points[currentPoint];
        vec3 to   = points[next];
        float segLen = length(to - from);
        // Segmento de comprimento zero (pontos duplicados): nao avanca para evitar nan
        if (segLen > 1e-5f) t += (speed / segLen) * deltaTime;
        if (t >= 1.0f) {
            t -= 1.0f;
            currentPoint = next;
...
ObjModel loadModel(const string& name, const string& objPath,
    ...
    // Tenta carregar a trajetoria do arquivo TRAJ_DIR/<name>.txt
    obj.trajectory.load(string(TRAJ_DIR) + name + ".txt");
    return obj;
}
...
// Quando a trajetoria esta ativa, sobrescreve obj.position com o ponto interpolado.
static void updateTrajectories(float deltaTime) {
    for (ObjModel& obj : models) {
        if (obj.trajectory.enabled && obj.trajectory.points.size() >= 2)
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT,
        "Desafio M6 - Trajetorias Ciclicas", nullptr, nullptr);
...
void keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    switch (key) {
        ...
        case GLFW_KEY_V:
            // Toggla a trajetoria ciclica do objeto selecionado
            obj.trajectory.enabled = !obj.trajectory.enabled;
            cout << "Trajetoria " << (obj.trajectory.enabled ? "ATIVA" : "INATIVA")
                 << " (" << obj.trajectory.points.size() << " pontos)" << endl;
            break;
