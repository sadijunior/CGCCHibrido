/*
 * Vivencial M4 - Iluminacao Tres Pontos
 * Modulo 4 - Computacao Grafica
 */
...
const vec3 CAMERA_POS    = vec3(0.0f, 0.0f, 2.5f);
const vec3 CAMERA_TARGET = vec3(0.0f, 0.0f, 0.0f);
const vec3 CAMERA_UP     = vec3(0.0f, 1.0f, 0.0f);

// Constantes de atenuacao (mesmo set de OpenGL classico)
const float ATT_KC = 1.0f;
const float ATT_KL = 0.09f;
const float ATT_KQ = 0.032f;
...
// Configuracao das tres luzes (key/fill/back)
struct Light {
    vec3  position;
    vec3  color;
    float intensity;
...
void main() {
    ...
    // Ambiente unico (somado uma unica vez, nao por luz)
    vec3 result = ka * vec3(0.3);

    for (int i = 0; i < 3; i++) {
        if (lightEnabled[i] == 0) continue;

        vec3  Lv = lightPos[i] - vWorldPos;
        float d  = length(Lv);
        vec3  L  = Lv / d;

        // Atenuacao aplicada no termo difuso
        float att = 1.0 / (Kc + Kl * d + Kq * d * d);

        vec3 diffuse = kd * max(dot(N, L), 0.0)
                       * lightColor[i] * lightIntensity[i] * att;

        vec3 R = reflect(-L, N);
        // max(ns,1.0) evita pow(0,0) indefinido
        vec3 specular = ks * pow(max(dot(R, V), 0.0), max(ns, 1.0))
...
// ============================================================================
// LUZES (key, fill, back) posicionadas em torno do objeto selecionado
// ============================================================================

static void updateLights() {
    const ObjModel& target = models[selectedModel];
    vec3 base = target.position;
    vec3 s    = target.scale;

    // Key light: forte, vinda de cima/direita/frente
    lights[0].position  = base + vec3( 1.5f,  1.0f,  1.5f) * s;
    lights[0].color     = vec3(1.0f, 1.0f, 1.0f);
    lights[0].intensity = 1.0f;

    // Fill light: suave, vinda do lado oposto, levemente azulada
    lights[1].position  = base + vec3(-1.5f, -0.5f,  1.0f) * s;
    lights[1].color     = vec3(0.7f, 0.8f, 1.0f);
    lights[1].intensity = 0.4f;

    // Back/rim light: atras do objeto, levemente quente
    lights[2].position  = base + vec3( 0.0f,  1.0f, -1.8f) * s;
    lights[2].color     = vec3(1.0f, 0.9f, 0.8f);
    lights[2].intensity = 0.6f;
}

static void uploadLights(GLuint shaderID) {
    vec3  positions[3];
    vec3  colors[3];
    float intensities[3];
    int   enabled[3];
    for (int i = 0; i < 3; i++) {
        ...
        enabled[i]     = lights[i].enabled ? 1 : 0;
    }
    glUniform3fv(glGetUniformLocation(shaderID, "lightPos"),       3, value_ptr(positions[0]));
    glUniform3fv(glGetUniformLocation(shaderID, "lightColor"),     3, value_ptr(colors[0]));
    glUniform1fv(glGetUniformLocation(shaderID, "lightIntensity"), 3, intensities);
    glUniform1iv(glGetUniformLocation(shaderID, "lightEnabled"),   3, enabled);
}
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT,
        "Vivencial M4 - Iluminacao Tres Pontos", nullptr, nullptr);
    ...
    // View e projection sao constantes nesse programa
    mat4 view       = lookAt(CAMERA_POS, CAMERA_TARGET, CAMERA_UP);
    mat4 projection = perspective(radians(45.0f),
                                  (float)WIDTH / (float)HEIGHT,
                                  0.1f, 100.0f);
    glUniformMatrix4fv(viewLoc, 1, GL_FALSE, value_ptr(view));
    glUniformMatrix4fv(projLoc, 1, GL_FALSE, value_ptr(projection));
    glUniform3fv(viewPosLoc, 1, value_ptr(CAMERA_POS));

    // Constantes de atenuacao
    glUniform1f(glGetUniformLocation(shaderID, "Kc"), ATT_KC);
    glUniform1f(glGetUniformLocation(shaderID, "Kl"), ATT_KL);
    glUniform1f(glGetUniformLocation(shaderID, "Kq"), ATT_KQ);
    ...
    // Tres luzes inicialmente ligadas
    for (int i = 0; i < 3; i++) lights[i].enabled = true;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Reposiciona luzes a cada frame, acompanhando o objeto selecionado
        updateLights();
